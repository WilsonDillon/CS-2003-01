class NQueens {
	...
	// TO COMPLETE
        board[row][column] = QUEEN; //sets Queen
        numPlacedQueens++; //increments teh number of queens placed
        markBoard(row, column, EMPTY, column+1); // marks each space this queen is attacking
    }  // end setQueen
	...
	// TO COMPLETE
        board[row][column] = EMPTY; // removes the queen
        markBoard(row, column, column+1, EMPTY); // removed the markers on the space that queen was attacking
    }  // end removeQueen
    ...
    /**
     * Marks the column number <code>col+distance</code> to denote the
     * location that can be attacked by the queen being placed.  <li>
     * Precondition: A queen was placed in the position (row, col) and
     * the (col+distance)th column is being marked as part of the
     * lookahead process.  <li> Postcondition: Marks up to three
     * squares on the board in column (col+distance).  These squares are
     ...
     * Calls itself recursively to mark the following columns.
     * @param row where the new queen is located
	...
	// TO COMPLETE
        if (col + distance < BOARD_SIZE) {
            ...
            }// marks empty spaces in the upper diagonal direction
            if (board[row][col+distance] == oldMarker) {
                board[row][col+distance] = newMarker;
            }// marks empty spaces in the right direction
            if (row + distance < BOARD_SIZE && board[row+distance][col+distance] == oldMarker) {
                board[row+distance][col+distance] = newMarker;
            }// marks empty spaces in the lower diagonal direction
            markForward(row, col, distance + 1, oldMarker, newMarker); // calls itself recursively, incrementing the distance
        }// end if
    }
    /**
	...
	return 
	    ...
	    +"Number of isAttacked() calls : "+isAttackedCalls +"<br>"
